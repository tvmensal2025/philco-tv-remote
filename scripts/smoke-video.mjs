...
function run(command, args) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) =>
      code === 0 ? resolve() : reject(new Error(`${command} (${code}): ${error}`)),
    );
  });
}
...
try {
  ...
  for (let index = 0; index < colors.length; index++) {
    ...
    const inputs = [
      ...
      'lavfi',
      '-i',
      `color=c=${colors[index]}:s=640x360:d=6`,
    ];
    if (index === 0)
      inputs.push('-f', 'lavfi', '-i', 'sine=frequency=440:duration=6', '-c:a', 'aac', '-shortest');
  ...
  const { renderVertical, probeDuration, hasAudioStream } =
    await import('../apps/worker/dist/pipeline/ffmpeg.js');
