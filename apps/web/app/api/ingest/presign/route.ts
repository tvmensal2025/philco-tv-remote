...
import { ingestPresignSchema, cameraStoragePrefix, rawSegmentPath } from '@reelops/shared';
...
export async function POST(request: Request) {
  try {
    ...
    if (authorization !== `Bearer ${env.INGEST_API_KEY}`)
    ...
    if (Math.abs(Date.now() - capturedAt.getTime()) > 24 * 60 * 60 * 1000)
      return NextResponse.json(
        { error: 'Timestamp do segmento fora da janela permitida.' },
        { status: 422 },
      );
    ...
    if (camera.storage_prefix !== canonicalPrefix)
      return NextResponse.json({ error: 'Prefixo da câmera não é canônico.' }, { status: 409 });
    ...
    return NextResponse.json({
      ...
      cameraId: camera.id,
      expiresIn: 600,
      completeUrl: `${env.APP_URL}/api/ingest/complete`,
    });
  } catch (error) {
    return NextResponse.json(
