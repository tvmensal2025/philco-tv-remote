...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: reel } = await ctx.supabase
      .from('reels')
      .select('id,output_path,thumbnail_path')
      .eq('id', id)
      .eq('tenant_id', ctx.tenantId)
      .single();
    if (!reel) return publicApiError('MEDIA_NOT_FOUND', 'Mídia não encontrada.', 404);
    const objectPath = type === 'thumbnail' ? reel.thumbnail_path : reel.output_path;
    if (!objectPath) return publicApiError('MEDIA_NOT_READY', 'Arquivo ainda não disponível.', 404);
    if (!isTenantMediaPath(objectPath, ctx.tenantId))
      return publicApiError('MEDIA_FORBIDDEN', 'Caminho de mídia inválido.', 403);
    ...
    if (message === 'UNAUTHORIZED') return publicApiError('UNAUTHORIZED', 'Não autorizado.', 401);
    return publicApiError('VIDEO_MEDIA_FAILED', 'Não foi possível abrir a mídia.', 400);
  }
}
