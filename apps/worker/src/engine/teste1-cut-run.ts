...
const source = 'D:\\DEV\\TESTE1.mp4';
const inspectDir = path.join(quality, 'inspect-teste');

if (!existsSync(source)) {
  throw new Error(`TESTE1_MISSING:${source}`);
}

const cameraId = 'e1e1e1e1-1111-4111-8111-111111111111';
const recordingId = 'e1e1e1e1-2222-4222-8222-111111111111';
...
const parsed = await analyzeMomentFramesOpenAI({
  ...
  cameras: [1],
  prompt:
    'Uma única câmera. Descreva o que está na imagem. Não invente restaurante, prato ou cidade. Não invente outras câmeras.',
});
...
try {
  const ai = await decideWithAiDirector({
    ...
    ids: {
      ...
      momentId: 'd00d0000-1111-4222-8333-666666666666',
      reelId: 'ffffffff-aaaa-4bbb-8ccc-dddddddddddd',
    },
...
const output = path.join(quality, 'quality-teste1-cut.mp4');
...
const report = {
  stock: 'D:\\DEV\\TESTE1.mp4 only',
  note: 'Single-camera cut. Not restaurant CCTV. Not FIRST PREMIUM REEL VALIDATED.',
...
writeFileSync(path.join(quality, 'teste1-cut.json'), JSON.stringify(report, null, 2));
